...
var lessonnumber = Number(url.split("/lesson/")[1]);
  ...
  fs.readFile("index.html", 'utf8', function(error, data) {
    ...
    data = localize(data,cookies.lang,{"SIDEBAR": renderSidebar(cookies.lang,cookies.code,lessonnumber), "TITLE": "{general.lesson}"+lessonnumber, "CODE": String(cookies.code).substring(0,4)+" "+String(cookies.code).substring(4,8)+" "+String(cookies.code).substring(8,12), "CONTENT": localize(lessontemplate,cookies.lang,{})})
    if (!data) {
      return internalServerError();
    }
    res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Content-Length': data.length, 'Cache-Control': 'private, max-age=0' });
